class Merchant < ApplicationRecord
  ...
  def total_revenue
    Merchant.joins(invoices: {invoice_items: {invoice: :transactions}}).where("invoices.status='shipped' AND transactions.result='success' AND merchants.id = ?", id).group("merchants.id").select("merchants.*, SUM(invoice_items.quantity * invoice_items.unit_price) AS revenue").first
  end
end
